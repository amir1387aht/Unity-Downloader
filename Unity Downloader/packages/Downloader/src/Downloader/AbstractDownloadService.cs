...
public abstract class AbstractDownloadService : IDownloadService, IDisposable
{
    ...
    public Task<Stream> DownloadFileTaskAsync(DownloadPackage package, string address, FileStream filestream, CancellationToken cancellationToken = default)
    {
        return DownloadFileTaskAsync(package, new[] { address }, filestream, cancellationToken);
    }
    ...
    public virtual async Task<Stream> DownloadFileTaskAsync(DownloadPackage package, string[] urls, FileStream filestream, CancellationToken cancellationToken = default)
    {
        Package = package;
        await InitialDownloader(cancellationToken, urls);
        return await StartDownload(filestream).ConfigureAwait(false);
    }
    ...
    protected async Task<Stream> StartDownload(FileStream filestream)
    {
        Package.FileName = filestream.Name;

        await StartDownloadWithFileStream(filestream).ConfigureAwait(false);
        return filestream;
    }

    protected abstract Task<Stream> StartDownload();

    protected abstract Task<Stream> StartDownloadWithFileStream(FileStream filestream);
